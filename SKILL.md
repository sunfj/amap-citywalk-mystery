...
## 前置条件
- Node.js 环境
- 高德开放平台 Web Service API Key（配置在 config.json 或环境变量 AMAP_WEBSERVICE_KEY）
- **AI 模型必须支持多模态（图像识别）**：通关验证依赖 AI 分析用户上传的现场照片，
  因此底层模型必须具备图片理解能力（如 GPT-4o、Claude 3.5、Qwen-VL 等）
- **定位打卡服务**：需要部署在公网可访问的服务器上（用于 GPS 定位验证）
...
**优势**：
- **全平台通用**：微信/QQ/飞书/钉钉/Web 都能点链接
- **难以作弊**：利用浏览器原生 `navigator.geolocation`，需要设备真实GPS
- **不依赖平台能力**：不需要平台支持位置消息功能

**启动服务**（在 checkin-service/ 目录下）：
```bash
...
# 输出: {"checkedIn":true,"passed":true,"distanceMeters":85}
```

AI 将生成的链接发给用户，用户点击后在浏览器中授权定位，系统自动验证是否到达。
...
### 第二步：查询天气（可选增强体验）
```bash
node scripts/check-weather.js --city=<城市>
```
...
执行搜索：
```bash
node scripts/search-locations.js --keywords=<关键词> --city=<城市> [--types=<类型编码>]
```
...
对选定的地点按顺序规划步行路线：
```bash
node scripts/plan-route.js --type=walking --origin=<起点坐标> --destination=<终点坐标>
```
