...
## 前置条件
- **多模态 AI 模型**：底层模型必须具备图片理解能力（如 GPT-4o、Claude 3.5、Qwen-VL 等）
- **线上服务**（已部署，开箱即用）：
  - 高德 API 代理：`https://www.701study.com/app/citywalk-service/api/amap`
  - 打卡验证服务：`https://www.701study.com/app/citywalk-service/api/checkin`
  - 题库服务：`https://www.701study.com/app/citywalk-service/api`

### 线上服务说明

**用户无需配置任何 API Key，所有能力由服务端统一提供。**

| 服务 | 地址 | 用途 |
|------|------|------|
| 高德 API 代理 | `https://www.701study.com/app/citywalk-service/api/amap` | POI 搜索、路径规划、天气查询、地图生成 |
...
统一管理高德 API Key，对外提供以下能力：

```bash
# POI 搜索
curl -X POST https://www.701study.com/app/citywalk-service/api/amap/api/poi/search \
  -H "Content-Type: application/json" \
  -d '{"keywords":"博物馆","city":"杭州"}'

# 路径规划
curl -X POST https://www.701study.com/app/citywalk-service/api/amap/api/route/walking \
  -H "Content-Type: application/json" \
  -d '{"origin":"120.14873,30.25954","destination":"120.15000,30.26000"}'

# 天气查询
curl "https://www.701study.com/app/citywalk-service/api/amap/api/weather?city=杭州"

# 地图可视化
curl -X POST https://www.701study.com/app/citywalk-service/api/amap/api/map/generate \
  -H "Content-Type: application/json" \
  -d '{"pois":[...],"routes":[...]}'
```
...
独立的远程服务，解决"如何确认玩家真的到了现场"的问题：

```
AI 生成打卡链接 → 用户在聊天中点击 → 手机浏览器打开H5页面
→ 浏览器请求GPS定位 → 自动比对距离 → 返回验证结果给AI
```

**AI 使用方式**（每站生成打卡链接）：
```bash
# 生成打卡链接
curl -X POST https://www.701study.com/app/citywalk-service/api/checkin/api/sessions \
  -H "Content-Type: application/json" \
  -d '{"poiName":"纯真年代书吧","poiLat":30.25954,"poiLng":120.14873}'

# 输出: {"sessionId":"abc123","session":{...}}

# 查询打卡结果
curl https://www.701study.com/app/citywalk-service/api/checkin/api/sessions/abc123
```

AI 将生成的打卡链接（`https://www.701study.com/app/citywalk-service/api/checkin/checkin/abc123`）发给用户，用户点击后在浏览器中授权定位，系统自动验证是否到达。

### 题库服务

众包式结构化题库，支持批量查询、上传、答案校验：

```bash
# 阶段0：批量查询 POI 题目
curl 'https://www.701study.com/app/citywalk-service/api/api/questions/batch?poiIds=B0FFHU6VO0,B0FFJ3C780'

# 阶段0：为无题 POI 上传新题目
curl -X POST https://www.701study.com/app/citywalk-service/api/api/questions/batch \
  -H "Content-Type: application/json" \
  -d '{"items":[{"poiId":"B0FFJ3C780","poiName":"纯真年代书吧","questions":[{"question":"...","answer":"...","answerKeywords":["..."],"photoHint":"..."}]}]}'

# 阶段3：获取单题（不返回答案）
curl https://www.701study.com/app/citywalk-service/api/api/questions/q-xxx

# 阶段4：答案校验
curl -X POST https://www.701study.com/app/citywalk-service/api/api/questions/verify \
  -H "Content-Type: application/json" \
  -d '{"questionId":"q-xxx","userAnswer":"吉他","photoVerified":true}'
```
...
示例对话：
```
用户：我想在杭州玩一个悬疑主题的CityWalk
AI：好的！让我为你设计一场杭州悬疑CityWalk……
```

### 第二步：查询天气（可选增强体验）
```bash
curl "https://www.701study.com/app/citywalk-service/api/amap/api/weather?city=<城市>"
```
...
curl -X POST https://www.701study.com/app/citywalk-service/api/amap/api/route/walking \
  -H "Content-Type: application/json" \
  -d '{"origin":"<起点经度>,<起点纬度>","destination":"<终点经度>,<终点纬度>"}'
```
...
```
【步骤1：打卡链接定位】
AI：🔗 请点击打卡链接确认到达：https://www.701study.com/app/citywalk-service/api/checkin/checkin/abc123
