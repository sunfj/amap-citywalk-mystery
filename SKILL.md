...
```bash
# POI 搜索
curl -X POST https://www.701study.com/app/citywalk-service/api/amap/poi/search \
  -H "Content-Type: application/json" \
  -d '{"keywords":"博物馆","city":"杭州"}'

# 路径规划
curl -X POST https://www.701study.com/app/citywalk-service/api/amap/route/walking \
  -H "Content-Type: application/json" \
  -d '{"origin":"120.14873,30.25954","destination":"120.15000,30.26000"}'

# 天气查询
curl "https://www.701study.com/app/citywalk-service/api/amap/weather?city=杭州"

# 地图可视化
curl -X POST https://www.701study.com/app/citywalk-service/api/amap/map/generate \
  -H "Content-Type: application/json" \
  -d '{"pois":[...],"routes":[...]}'
```
...
```bash
# 生成打卡链接
curl -X POST https://www.701study.com/app/citywalk-service/api/checkin/sessions \
  -H "Content-Type: application/json" \
...
# 查询打卡结果
curl https://www.701study.com/app/citywalk-service/api/checkin/sessions/abc123
```

AI 将生成的打卡链接（`https://www.701study.com/app/citywalk-service/checkin/abc123`）发给用户，用户点击后在浏览器中授权定位，系统自动验证是否到达。
...
```bash
# 阶段0：批量查询 POI 题目
curl 'https://www.701study.com/app/citywalk-service/api/questions/batch?poiIds=B0FFHU6VO0,B0FFJ3C780'

# 阶段0：为无题 POI 上传新题目
curl -X POST https://www.701study.com/app/citywalk-service/api/questions/batch \
  -H "Content-Type: application/json" \
  -d '{"items":[{"poiId":"B0FFJ3C780","poiName":"纯真年代书吧","questions":[{"question":"...","answer":"...","answerKeywords":["..."],"photoHint":"..."}]}]}'

# 阶段3：获取单题（不返回答案）
curl https://www.701study.com/app/citywalk-service/api/questions/q-xxx

# 阶段4：答案校验
curl -X POST https://www.701study.com/app/citywalk-service/api/questions/verify \
  -H "Content-Type: application/json" \
...
### 第四步：查询天气（可选增强体验）
```bash
curl "https://www.701study.com/app/citywalk-service/api/amap/weather?city=<城市>"
```
